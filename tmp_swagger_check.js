 ...
 *
 * Server URL priority:
 *   1. process.env.HOST  – set this on Render/production (e.g. https://mashawerr-api.onrender.com)
...
const options = {
    definition: {
        ...
        },      // closes components (extra needed due to securitySchemes nesting)
        paths: {
            '/api/auth/register': {
                post: {
                    tags: ['Auth'],
...
Internally (KD): \`total = (baseFare + distance_meters × pricePerMeter) × surgeMultiplier\`; then \`price = round(total × 1000)\` fils. Includes \`name_ar\`.`,
            ...
            '/api/store/products': {
                get: {
                    ...
                    description: 'Returns products. **Clients** see active products only, without stock qty. **Admin/Agent** see everything including stock.',
...
// All paths are defined inline above (not in JSDoc comments).
// swagger-jsdoc is designed for JSDoc annotations and ignores inline paths.
// So we export options.definition directly for swagger-ui-express.
global.__def = options.definition;
